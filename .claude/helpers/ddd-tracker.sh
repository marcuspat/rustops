...
# V3 Target Domains - RustOps Bounded Contexts
DOMAINS=(
  "telemetry-collection"      # Telemetry Collection context
  "anomaly-detection"         # Anomaly Detection context
  "incident-management"       # Incident Management context
  "remediation"               # Remediation context
  "service-topology"          # Service Topology context
  "integration"               # Integration context
  "knowledge-management"      # Knowledge Management context
)
...
check_domain() {
  local domain="$1"
  local domain_path="$PROJECT_ROOT/crates/$domain"
  local alt_path="$PROJECT_ROOT/src/domains/$domain"
  local plans_path="$PROJECT_ROOT/plans"

  local score=0
  local max_score=100

  # Check if domain has ADR/documentation (25 points)
  local adr_count=$(find "$plans_path/adrs" "$plans_path/ddd" -name "*${domain}*" -o -name "*$(echo $domain | sed 's/-/ /g')*" 2>/dev/null | wc -l)
  [ "$adr_count" -gt 0 ] && score=$((score + 25))
  ...
  # Check for tests (10 points)
  local test_count=$(find "$path" "$PROJECT_ROOT/tests" -name "*${domain}*.test.rs" -o -name "*${domain}*_test.rs" 2>/dev/null | wc -l)
